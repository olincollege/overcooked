...
class ViewCook():
    ...
    def draw_timer(self):
        ...
        self.model.world.blit(text, self.timer_coord)  # Draw the text on the screen
    ...
    def draw_cooker_timer(self):
        ...
        if self.model._stove == True:
            elapsed_time = int((pygame.time.get_ticks() - self.model.cooking_timer)/1000)
            font = pygame.font.Font(None, 30)  # Choose the font for the tex_coord
            if elapsed_time < 5:
                text = font.render(str(elapsed_time), 1, self.BLACK)  # Create the text
            elif elapsed_time > 8:
                text = font.render(str('Overcooked'), 1, self.BLACK)  # Create the text
            else:
                text = font.render(str('Done'), 1, self.BLACK)  # Create the text
            self.model.world.blit(text, self.cooker_timer_coord)  # Draw the text on the screen
    ...
    def draw_money(self):
        ...
        text = font.render(str(self.model._geld), 1, self.BLACK)  # Create the text
    ...
    def draw_money_end(self):
        ...
        """
        font = pygame.font.Font(None, 60)  # Choose the font for the text
        text = font.render(str(self.model._geld), 1, self.WHITE)  # Create the text
