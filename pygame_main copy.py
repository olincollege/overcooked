...
while MAIN:
    for event in pygame.event.get(): #every time an event happens in pygame
        ...
        if counter >= 0: #as long as the game isn't over
            if event.type == pygame.KEYDOWN: #whenever there is a key press
                # If the key press is ADWS or an arrow key, start moving the player
                if event.key == pygame.K_LEFT or event.key == ord('a'):
                    controller.control(-STEP, 0)
                if event.key == pygame.K_RIGHT or event.key == ord('d'):
                    controller.control(STEP, 0)
                if event.key == pygame.K_UP or event.key == ord('w'):
                    controller.control(0, -STEP)
                if event.key == pygame.K_DOWN or event.key == ord('s'):
                    controller.control(0, STEP)
                
                if event.key == ord('f'): #If the key press is f
                    if 50 < player.rect.x < 250 and player.rect.y > 500: # if the player is at the plate station
                        model.isplate = True # player has a plate
                        view.player_appearance() # update the player image

                    if model.isplate: # if the player is holding a plate
                        if 50 < player.rect.x < 250 and player.rect.y < 315: # if the plater is at the first ingredient station
                            if player.plate[0] is False: #if they are not currently holding this ingredient
                                player.plate[0] = True # give them the ingredient

                        elif 355 < player.rect.x < 600 and player.rect.y < 315: #second ingredient station
                            if player.plate[1] is False:
                                player.plate[1] = True

                        elif 655 < player.rect.x < 910 and player.rect.y < 315: #third ingredient station
